...
const piecesList = Array({
	Name: "Pawn",
	Description: "A pawn can move one square directly forward. It cannot retreat."
}, {
	Name: "Promoted Pawn",
	Description: "Loses his current moveset and gains properties of a gold general."
}, {
	Name: "Lance",
	Description: "A lance can move any number of free squares directly forward. It cannot move backward or to the sides."
}, {
	Name: "Promoted Lance",
	Description: "Loses his current moveset and gains properties of a gold general."
}, {
	Name: "Bishop",
	Description: "A bishop can move any number of free squares along any one of the four diagonal directions."
}, {
	Name: "Promoted Bishop",
	Description: "It moves like a bishop or a king, but each turn only one of two ways."
}, {
	Name: "Rook",
	Description: "A rook can move any number of free squares along any one of the four orthogonal directions."
}, {
	Name: "Promoted Rook",
	Description: "It moves like a rook or a king, but each turn only one of two ways."
}, {
	Name: "Knight",
	Description: "Knight moves like chess knights, in an “big L” shape, but can only move forward."
}, {
	Name: "Promoted Knight",
	Description: "Loses his current moveset and gains properties of a gold general."
}, {
	Name: "Silver General",
	Description: "A silver general can move one square diagonally or one square directly forward, giving it five possibilities."
}, {
	Name: "Promoted Silver General",
	Description: "Loses his current moveset and gains properties of a gold general."
}, {
	Name: "Gold General",
	Description: "A gold general can move one square orthogonally, or one square diagonally forward, giving it six possible destinations. It cannot move diagonally backward."
}, {
	Name: "King",
	Description: "A king can move one square in any direction, orthogonal or diagonal."
}, );
...
Room.playerTurn = function (socket) {
	if (Room.list[socket.room].wantToStart == 0) {
		socket.broadcast.to(socket.room).emit(
			'askToStart', 
			"<span>Your opponent wants to start the match</span><button id=\"startMatchButton\" class=\"accept\">Accept</button>"
		);
		...
		<div class="modal">
      		<div class="modal-rules-box">
    			...
    			<div class="section">
        			<h2>General</h2>
        			<span>
        				Each player has twenty pieces: one King, two Gold Generals, two Silver Generals, two Knights, two Lances, 
								one Rook, one Bishop and nine Pawns. 
        		...
        		<div class="section">
        			...
        			<span>
            			...
            			<br>
            			Checkmate: If a player cannot avert a danger and his King will be taken anyway on the next turn, 
									this is checkmate and that player loses the game.
            			<br>
            			Repetition: the game ends if the same game position occurs four times consecutively – 
									the game is considered a draw. 
            			However, if an eternal check originated from this situation, 
									the player giving check and causing such a situation loses the game. 
            			The game reaches a jishogi if both kings have advanced into their respective promotion zones 
            			...
            			Each rook and bishop scores 5 points and all other pieces except kings score 1 point each. 
									A player scoring less than 24 points loses, otherwise the game is considered a draw.

            			Note: In professional and serious amateur games, 
									a player who makes an illegal move immediately loses the game.
          			</span>
        		</div>
		...
		rules += `</ul>
        		...
        		<div class="section">
          			...
          			<span>
            			...
            			So, when a piece moves into, out of or fully inside the promotion zone 
									it may be promoted upon completion of its move. 
            			Promotion is optional, provided that the piece still can make a legal move in case it is not promoted: 
									if a Pawn or a Lance move to the last row, or a Knight moves to either of the last two rows, 
									it must be promoted. 
        		...
        		<div class="section">
        			...
        			<span>
            			Pieces are captured as follows: 
									the attacking piece takes the place of the captured enemy piece (capturing by placement). 
            			Captured pieces are removed from the game board. 
            			<br>
            			A player who captured the piece acquires the piece and places it aside to be seen, 
									next to the game board on his side, and always shown as an unpromoted piece. 
            			Capturing pieces is optional. 
            			<br>
            			It is not mandatory to capture an enemy piece, 
									it is always up to the player to decide if and when to capture an opponent's piece.
        			</span>
        		</div>
        		...
        		<div class="section">
        			...
        			<span>
            			...
            			<br>
            			A player may drop a captured piece on any empty square on the board as his own 
									(that's why the pieces are differentiated only by their oriented tips, not by colors). 
            			<br>
            			By dropping a piece, a check can be given (endangering of the king does not have to be announced), 
									or even a checkmate can be given (except by a pawn). 
            			<br>
            			...
            			<br>
            			A piece should be dropped on a field where it is still possible to move such a piece for 
									at least one more move. 
									Pawns and lances cannot be dropped onto the last row while knights cannot be dropped on the last two rows. 
            			<br>
            			A pawn cannot be dropped in a column containing another unpromoted pawn of the same player 
									(promoted pawns do not count). 
            			<br>
            			A pawn cannot be dropped to give an immediate checkmate (although other pieces can); 
									however, pawns may give checkmate on any subsequent move after the pawn was dropped.
        			</span>
        		</div>
...
io.on('connect', function (socket) {
	...
	socket.on('movements', function (selected) {
		...
		if (room.turn == "black" && findObjectByTripleKey(
			room.player1Pieces, 
			'property', 'currentPosition', 'captured', 'player1', selected, false
			)) {

			moves = possibleMoves(room.player1Pieces, selected); // assign to possible moves

		} else if (room.turn == "white" && findObjectByTripleKey(
			room.player2Pieces, 
			'property', 'currentPosition', 'captured', 'player1', selected, false
			)) {
	...
	socket.on('move', function (selectedPiece, position) {
		...
		if (room.turn == "black") {
			...
			opponentPiece = findObjectByTripleKey(
				room.player2Pieces, 
				'property', 
				'currentPosition', 
				'captured', 
				'player2', 
				convertPosition(selectedPiece), 
				false
				);

			// check if i clicked onto an opponent piece
			eatedPiece = findObjectByTripleKey(
				room.player1Pieces, 
				'property', 
				'currentPosition', 
				'captured', 
			...
			if (eatedPiece) {
				...
				oppponentEatedPiece = findObjectByTripleKey(
					room.player2Pieces, 
					'property', 
					'currentPosition', 
					'captured', 
		...
		if (endGame == false && (extractCoordinates(position) >= 6 || 
		extractCoordinates(piece.currentPosition) >= 6) && 
		piece.promoted == false && piece.upgradable == true) {
			if (((piece.pieceName == "Pawn" || piece.pieceName == "Lance") && 
			extractCoordinates(position) == 8) || 
...
function surrender(socket) {
	...
	socket.emit(
		'endGame', 
		"<div class='modal'><div class='modal-box'><span>You lost</span><ul><li><button class='rematch' value='Rematch'>Rematch</button></li><li><button value='Quit' class='quit'>Quit</button></li></ul></div></div>"
		);
	socket.broadcast.to(socket.room).emit(
		'endGame', 
		"<div class='modal'><div class='modal-box'><span>You won</span><div class='end'><img src='/images/layout/crown.svg'></div><button value='Close' class='close'>Close</button></div></div>"
		);
}
